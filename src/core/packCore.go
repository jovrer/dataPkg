	...
	if this.checkPackStruct() {
		...
		for i = 0; i < len(this.fieldsPreix); i++ {
			...
			case VALUE_TYPE_STRING:
				datasTemp = NewUtils().String2Bytes(this.fieldsPreix[i].Value, this.fieldsPreix[i].Len)
				break
			case VALUE_TYPE_INT:
				datasTemp = NewUtils().Int2Bytes(this.fieldsPreix[i].Value, this.fieldsPreix[i].Len)
				break
			case VALUE_TYPE_BYTE:
				// datasTemp = NewUtils().bytesAlign(this.fieldsPreix[i].Value, this.fieldsPreix[i].Len)
				// break
			default:
				break
			}
		...
		for i = 0; i < len(this.fieldsSuffix); i++ {
			...
			case VALUE_TYPE_STRING:
				datasTemp = NewUtils().String2Bytes(this.fieldsSuffix[i].Value, this.fieldsSuffix[i].Len)
				break
			case VALUE_TYPE_INT:
				datasTemp = NewUtils().Int2Bytes(this.fieldsSuffix[i].Value, this.fieldsSuffix[i].Len)
				break
			case VALUE_TYPE_BYTE:
				// datasTemp = NewUtils().bytesAlign(this.fieldsSuffix[i].Value, this.fieldsSuffix[i].Len)
				// break
			default:
				break
			}
	...
	if this.checkPackStruct() {
		if this.pkgLen == len(datas) {
			...
			for i = 0; i < len(this.fieldsPreix); i++ {
				...
				switch this.fieldsPreix[i].ValueType {
				case VALUE_TYPE_STRING:
					this.fieldsPreix[i].Value = NewUtils().StringFromBytes(datas[bPos:this.fieldsPreix[i].Len])
					bPos += this.fieldsPreix[i].Len
					break
				case VALUE_TYPE_INT:
					this.fieldsPreix[i].Value = NewUtils().IntFromBytes(datas[bPos:this.fieldsPreix[i].Len])
					bPos += this.fieldsPreix[i].Len
					break
				case VALUE_TYPE_BYTE:
					// datasTemp = NewUtils().bytesAlign(this.fieldsPreix[i].Value, this.fieldsPreix[i].Len)
			...
			this.dataReal = datas[bPos:this.dataLen]

			for i = 0; i < len(this.fieldsSuffix); i++ {
				var datasTemp []byte
				switch this.fieldsSuffix[i].ValueType {
				case VALUE_TYPE_STRING:
					this.fieldsPreix[i].Value = NewUtils().StringFromBytes(datas[bPos:this.fieldsSuffix[i].Len])
					bPos += this.fieldsSuffix[i].Len
					break
				case VALUE_TYPE_INT:
					this.fieldsSuffix[i].Value = NewUtils().IntFromBytes(datas[bPos:this.fieldsSuffix[i].Len])
					bPos += this.fieldsSuffix[i].Len
					break
				case VALUE_TYPE_BYTE:
					// datasTemp = NewUtils().bytesAlign(this.fieldsSuffix[i].Value, this.fieldsSuffix[i].Len)
